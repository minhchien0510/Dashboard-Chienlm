...
def build_report(df, report_date, targets, report_type, filter_nv=None):
    ...
    total_row = pd.DataFrame([{
        'STT': '-',
        'Mã NVBH': 'TỔNG CỘNG',
        'Tên NVBH': 'SS Lê Minh Chiến Total' if filter_nv == "Tất cả ĐDKD" else filter_nv,
...
# ----- TAB DSKH_Combo OFF -----
with tab_dskh_off:
    st.markdown('<h3 style="color: #034ea2; font-weight: 800; margin-bottom: 0px; font-size: 15px;">📋 DANH SÁCH KHÁCH HÀNG COMBO OFF (CHIẾN_COMBO KÊNH OFF)</h3>', unsafe_allow_html=True)
...
# ----- TAB DSKH_Combo ON -----
with tab_dskh_on:
    st.markdown('<h3 style="color: #034ea2; font-weight: 800; margin-bottom: 0px; font-size: 15px;">📋 DANH SÁCH KHÁCH HÀNG COMBO ON (CHIẾN_COMBO KÊNH ON)</h3>', unsafe_allow_html=True)
